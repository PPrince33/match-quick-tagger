...
export default function App() {
  ...
  const endMatch = async () => {
    if (activeMatch) {
      // 1. Log the 'End' event before closing out the match
      const matchMinute = Math.floor(matchSeconds / 60);
      await supabase.from('events').insert([{
        match_id: activeMatch.id,
        team_id: null, // Sending null since the end of a match isn't specific to one team
        event_type: 'End',
        is_attacking_3rd: isAttacking3rd,
        match_minute: matchMinute
      }]);

      // 2. Update the match status to finished
      await supabase
        .from('matches')
